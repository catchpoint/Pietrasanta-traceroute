...
static void err_bad_excl(const CLIF_option* optn, char c, int n) 
{
    CLIF_option tmp = *optn;
    char* ss = NULL;
    char* excl = show_excl(curr.option_list, 0); /*  Note: show_(short|long)() nested!!! */
...
int CLIF_parse_cmdline(int argc, char *argv[], CLIF_option *option_list, CLIF_argument *argument_list, unsigned int parse_flags) 
{
    ...
    for(i = 1; i < argc; i++) {
        ...
        if(*arg != sym) {    /*  argument or keyword   */
            ...
            if(*arg == sym || (parse_flags & (_CLIF_STRICT_KEYWORD|_CLIF_STRICT_ONEDASH))) {    /*  `---' - let it be not option... */
                arg -= 2;
    ...
    if(num_args > 0) {
        ...
        if(j < num_argm - strict_end && !(argument_list[j].flags & CLIF_MORE) && (argument_list[j].flags & CLIF_ACC_PREV)) { /*  ...i.e, there are some missing optional args...  */
            if(j == 0)
                err_report("Incorrect argument list set: first arg cannot be `accompanied with previous'.");
            else
                err_report("Arg \"%s\" must be specified because \"%s\" `%s' is used.", argument_list[j].name, argument_list[j - 1].name, argv[arg_n[i - 1]]);
            return -1;
        }
        
        if(argm_index[--i] == j && ++j < num_argm - strict_end)
        ...
        for(i = 0; i < num_args; i++) {
            ...
            if(argm->function && argm->function (argm, argv[arg_n[i]], i) < 0) {
                err_report("Cannot handle \"%s\" cmdline arg `%s' on position %d (argc %d)", argm->name, argv[arg_n[i]], i + 1, arg_n[i]);
                return -1;
            }
        }
