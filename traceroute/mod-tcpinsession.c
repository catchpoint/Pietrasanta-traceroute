    ...
    for(int i = 0; i < n_flows; i++) {
        ...
        if((res && strlen(res) > 0) || (sack > 0 && SACK_permitted > 0) || ((mss > 0 || print_received_mss) && mss_received[i] > 0) || print_five_tuple > 0) {
            ...
            if(print_five_tuple) {
                ...
                printf("%s%s%s:%u->%s%s%s:%u", (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", src_str, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(src[i].sin.sin_port), (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", dest_str, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(dest_addr.sin.sin_port));
            }
...
static probe* tcpinsession_check_reply(int sk, int err, sockaddr_any* from, char* buf, size_t len) 
{
        ...
        if(pb && print_five_tuple && pb->five_tuple == NULL) {
            for(int i = 0; i < n_flows; i++) {
                if(tcp->source == src[i].sin.sin_port) {
                    ...
                    snprintf(dest_str, sizeof(dest_str), "%s", addr2str(&dest_addr));
                    snprintf(str, sizeof(str), "%s->%s%s%s:%u", src_str, (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", dest_str, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(dest_addr.sin.sin_port));
    ...
    if(print_five_tuple && pb->five_tuple == NULL) {
        ...
        snprintf(dest_str, sizeof(dest_str), "%s", addr2str(&dest_addr));
        snprintf(str + strlen(str), sizeof(str) - strlen(str), "%s->%s%s%s:%u", src_str, (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", dest_str, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(dest_addr.sin.sin_port));
