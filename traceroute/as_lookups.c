...
const char* get_as_path(const char* query) 
{
    ...
    int prefix = 0;
    int best_prefix = 0;
    char* re = &ra_buf[sizeof(ra_buf) / sizeof(*ra_buf) - 1];

    if(!ra_addr.sa.sa_family) {
        const char* server = getenv("RA_SERVER");
        if(!server)
            server = DEF_RADB_SERVER;
            
        const char* service = getenv("RA_SERVICE");
        if(!service)
            service = DEF_RADB_SERVICE;
            
        struct addrinfo* res;
        int ret = getaddrinfo(server, service, NULL, &res);
        if(ret) {
    ...
    int sk = socket(ra_addr.sa.sa_family, SOCK_STREAM, 0);
    if(sk < 0)
        error("socket");
