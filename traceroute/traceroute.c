...
uint16_t prepare_ancillary_data(struct iphdr* outer_ip, struct icmphdr* outer_icmp, struct iphdr* inner_ip, uint16_t inner_proto_hlen, struct msghdr* ret)
{
    ...
    // In summary, it appears that CMSG_LEN is used to indicate the amount to read, while CMSG_SPACE is used to reserve the actual space 
    ...
    *data_ip_ttl = outer_ip->ttl;
    ...
    // Also, et info like icmp_unreach@icmp.c:801
    ...
    switch(outer_icmp->type)
    {
        ...
        case ICMP_DEST_UNREACH:
        {
            ...
            // See raw_err@raw.c
            err = EHOSTUNREACH;
            if(outer_icmp->code > sizeof(icmp_err_convert) / sizeof(struct icmp_err) - 1) // NR_ICMP_UNREACH is defined as 15 in include/net/icmp.
                break;
            err = icmp_err_convert[outer_icmp->code].error;
...
uint16_t prepare_ancillary_data_v6(sockaddr_any* offender, struct icmp6_hdr* outer_icmp, struct ip6_hdr* inner_ip, uint16_t inner_proto_hlen, struct msghdr* ret)
{
    ...
    // Note that we need to put the OUTER IP TTL
    *data_ip_ttl = 0; //outer_ip->ttl; // Cannot do that without the IP header. This means that we can't use "backward" tr option.
    ...
    switch(outer_icmp->icmp6_type)
    {
        ...
        case ICMP6_DST_UNREACH:
        {
            ...
            err = EPROTO;
            if(outer_icmp->icmp6_code > sizeof(tab_unreach)/sizeof(struct icmp6_err)) 
                break;
            err = tab_unreach[outer_icmp->icmp6_code].err;
...
printf("Received message\n");
    ...
    */
    if(!err && ops->is_raw_icmp_sk(sk) == 0 && af == AF_INET && ops->header_len == 0) {
