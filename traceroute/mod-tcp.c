...
static void tcp_send_probe(probe* pb, int ttl, int probe_idx)
{
    ...
    */
    ...
    if(print_five_tuple) {
        ...
        snprintf(source_address, sizeof(source_address), "%s", addr2str(&src));
        snprintf(destination_address, sizeof(destination_address), "%s", addr2str(&dest_addr));
        snprintf(five_tuple, sizeof(five_tuple), "%s%s%s:%u->%s%s%s:%u", (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", source_address, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(th->source), (dest_addr.sa.sa_family == AF_INET6) ? "[" : "", destination_address, (dest_addr.sa.sa_family == AF_INET6) ? "]" : "", ntohs(dest_port));
